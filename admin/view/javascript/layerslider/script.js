...
(function( $ ) {
	...
	$.fn.pavoSliderEditor = function( initvar ) {
		...
		this.process = function( siteURL , adminURL, delayTime ) {
			...
			$('input, select ,textarea', '#slider-form' ).change( function(){  
				if( $(this).attr('name') =='layer_top' || $(this).attr('name') == 'layer_left' ) { 
					var position = self._getLayerPosition($this.currentLayer.width(), $this.currentLayer.height(), $('[name="layer_left"]').val(), $('[name="layer_top"]').val());
	 		...
	 		if( data !=null && data ) { 
				...
				data['layer_caption'] = data['layer_caption'].replace(/_ASM_/,'&');
				 ...
				 $( '[name="layer_time['+data['layer_id']+']"]','#slider-form' ).val( data['time_start'] );
	 			 ...
	 			 $this.currentLayer.find('img').on('load', function () {
	 			 	var position = $this._getLayerPosition($layer.width(), $layer.height(), data['layer_left'], data['layer_top']);
		 ...
		 */
	 	...
	 	this._getLayerPosition = function (layerWidth, layerHeight, left, top, hleft, htop) {
	 		var self = this,
	 			previewWidth = self.seditor.width(),
	 			previewHeight = self.seditor.height(),
	 			hleft = hleft || 0,
	 			htop = htop || 0,
	 			layerX, layerY;

	 		if (left == 'left' || left == 'right' || left == 'center') {
	 			if (left == 'left') {
	 				layerX = 0;
	 			} else if (left == 'right') {
	 				layerX = previewWidth - layerWidth;
	 			} else {
	 				layerX = (previewWidth - layerWidth ) / 2;
	 			}
	 		} else {
	 			layerX = left;
	 		}
	 		...
	 		if (top == 'top' || top == 'bottom' || top == 'center') {
	 			if (top == 'top') {
	 				layerY = 0;
	 			} else if (top == 'bottom') {
	 				layerY = previewHeight - layerHeight;
	 			} else {
	 				layerY = (previewHeight - layerHeight) / 2;
	 			}
	 		} else {
	 			layerY = top;
	 		}
