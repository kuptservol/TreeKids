...
class ModelModuleKbm extends Model
{
	...
	{
		return date(_t('date_format_short') . ' ' . _t('time_format'), $time);
	}

	public function formatDate($time)
	{
		return date(_t('date_format_short'), $time);
	}

	public function formatDatabaseDateTime($time)
	{
		return date('Y-m-d H:i', $time);
	}

	public function formatDatabaseDate($time)
	{
    ...
    public function convertDateTimeToTimestamp($date_formatted)
    {
	    ...
	    {
		    $dt = DateTime::createFromFormat('Y-m-d H:i', preg_replace('/\b(\d)\b/', '0$1', $date_formatted));
    ...
    public function convertDateToTimestamp($date_formatted)
    {
	    if (version_compare(phpversion(), '5.3.0', '>='))
	    {
		    $dt = DateTime::createFromFormat('Y-m-d', preg_replace('/\b(\d)\b/', '0$1', $date_formatted));
		    $timestamp = $dt->getTimestamp();
	    }
	    else
	    {
